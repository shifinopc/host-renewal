@php($pageTitle = 'Edit Domain')
@php($pageSubtitle = 'Update domain configuration and renewal details')

@extends('layouts.app')

@section('content')
    <div class="max-w-3xl" x-data="{ domainName: @js(old('domain_name', $domain->domain_name)), faviconLoaded: false }" x-effect="domainName; faviconLoaded = false">
            ...
            <form method="POST" action="{{ route('domains.update', $domain) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input name="plan_name" value="{{ old('plan_name', $domain->plan_name) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input name="price" type="number" step="0.01" value="{{ old('price', $domain->price) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Start Date</label>
                        <input name="start_date" type="date" value="{{ old('start_date', optional($domain->start_date)->format('Y-m-d')) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Expiry Date</label>
                        <input name="expiry_date" type="date" value="{{ old('expiry_date', optional($domain->expiry_date)->format('Y-m-d')) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Notes</label>
                    <textarea name="notes" rows="3" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $domain->notes) }}</textarea>
                </div>
