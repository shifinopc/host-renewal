<form method="POST" action="{{ route('proforma-invoices.store') }}" class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
            ...
            <input name="amount" type="number" step="0.01" value="{{ old('amount') }}" required class="w-full rounded-xl border border-slate-300 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
        <div>
    ...
    <div class="flex items-center justify-end gap-3 pt-2">
        <button type="button" @click.prevent="$dispatch('close-proforma-create')" class="text-xs text-slate-500">Cancel</button>
