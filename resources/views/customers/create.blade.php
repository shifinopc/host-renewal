...
@php($pageSubtitle = 'Create a new customer profile')
...
@section('content')
    <div class="max-w-xl">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <form method="POST" action="{{ route('customers.store') }}" class="space-y-4" x-data="{ taxPreference: '{{ old('tax_preference', 'taxable') }}' }">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="company" value="{{ old('company') }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Phone</label>
                        <input name="phone" value="{{ old('phone') }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Email</label>
                    <input name="email" type="email" value="{{ old('email') }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Address</label>
                    <textarea name="address" rows="3" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('address') }}</textarea>
                </div>
                ...
                <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="business_type" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                @foreach($businessTypes as $code => $label)
                                    <option value="{{ $code }}" @selected(old('business_type') == $code)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">GSTIN / Unit (optional)</label>
                            <input name="gstin" value="{{ old('gstin') }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            ...
                            <select name="place_of_supply" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                @foreach($indianStates as $code => $label)
                                    <option value="{{ $code }}" @selected(old('place_of_supply') == $code)>{{ '[' . $code . '] - ' . $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div x-show="taxPreference === 'exempt'">
                        ...
                        <textarea name="tax_exempt_reason" rows="2" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('tax_exempt_reason') }}</textarea>
                    </div>
                </div>
